...
exports.createMateri = async (req, res) => {
	try {
		...
		if (!judul) {
			return res
				.status(400)
				.json({ message: "Field 'judul' harus disediakan." });
		}
		if (!guru_id) {
			return res
				.status(400)
				.json({ message: "Field 'guru_id' harus disediakan." });
		}
		if (!mapel_id) {
			return res
				.status(400)
				.json({ message: "Field 'mapel_id' harus disediakan." });
		}
		...
		if (!guru) {
			return res
				.status(400)
				.json({ message: "Guru dengan ID tersebut tidak ditemukan." });
		}
		if (!mapel) {
			return res
				.status(400)
				.json({ message: "Mapel dengan ID tersebut tidak ditemukan." });
		}
		...
		if (error.code === "P2002") {
			return res
				.status(400)
				.json({ message: "Data dengan nilai ters  ebut sudah ada." });
		}
